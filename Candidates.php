...
if(isset($_POST['add'])){
    ...
    $conn->query("INSERT INTO Candidates (candFName, candMName, candLName, posID, candStat) 
    VALUES ('$candFName', '$candMName', '$candLName', '$posID', '$candStat')"); 
}
...
if(isset($_POST['edit'])){
    ...
    $posID = $_POST['posID'];
    $candStat = $_POST['candStat'];
    $conn->query("UPDATE Candidates SET candFName='$candFName', candMName='$candMName', 
    candLName='$candLName', posID='$posID', candStat='$candStat' WHERE candID=$id");
}
...
$positions_edit = $conn->query("SELECT * FROM Positions WHERE posStat='open'");
$positions_add = $conn->query("SELECT * FROM Positions WHERE posStat='open'");
...
<body>
    ...
    <form method="post">
        ...
        POSITION :
        <SELECT name="posID">
            <?php while($p = $positions_edit->fetch_assoc()):?>
                <option value="<?=$p['posID']?>" <?=$edit_candidate['posID']==$p['posID'] ? 'selected' : ''?>><?=$p['posName']?></option>
            <?php endwhile;?>    
        </SELECT><br>
        STATUS :
    ...
    <form method="post">
        ...
        POSITION :
        <SELECT name="posID">
            <?php while($p = $positions_add->fetch_assoc()):?>
